...
def first_page(folder_tmp,location_name,target_lat,target_lon,tide_name):
    ...
    ax.text(0.01, 1.07,"Contact: contactus@met.gov.tv", transform=ax.transAxes,fontsize=6, verticalalignment='top', color='#2F5F8A')
    ax.axis('off')
    axline = fig.add_axes([0.001, 0.90, 0.99, 0.02])
    axline.axhline(y=0.5, color='#2F5F8A', linestyle='-',linewidth = 15)
    axline.axis('off')
    issue_time = datetime.datetime.now().strftime("%Y-%m-%d %H:%M")
	...
	#orginalspeed = WSPD.values
    gust_df = gust_df.values * 1.9438452
    #gust_df.values * (10/0.1)**(1/7)) # to adjust surface wind to 10 m wind
    WSPD = WSPD.multiply(1.9438452)
    windspeedx = WSPD.values
    ax2.plot(time.values, windspeedx,color='black',label='Wind Speed')
    ax2.plot(time.values, gust_df,color='black',label='Gusts',linestyle='dashed')
    ...
    for x in time:
        ...
        if is_multiple_of_three(i):
            ax2.quiver(x, windspeedx[i], change[0], change[1], color=windcolor(windspeedx[i]),scale=25,width=0.01, edgecolor='black', linewidth=0.7,headwidth=2.5, headlength=2.5, headaxislength=2.4)

        i+=1
    setupAxis(ax2)
    ax2.set_ylim([windspeedx.min()-10,windspeedx.max()+20])
    ...
    #swell =hs.values*0.60
    #sea = hs.values*0.40
    ax11.plot(time.values, hs.values, color='black', label='Sig Wave Height')
    ax11.set_ylabel("Wave Height [m]",fontsize=7)
    ax11.set_ylim([0,hs.max()+0.5])
    setupAxis(ax11)
    ...
    ax33.grid(True)
    ax33.plot(time.values,peakperiod.values, color='black', label='Peak Period')
    ...
    for x in time:
        ...
        change = [speed * math.cos(angle), speed * math.sin(angle)]
        if is_multiple_of_three(i):
            ax33.quiver(x, peakperiod[i], change[0], change[1], color='lightblue',scale=25,width=0.01, edgecolor='black', linewidth=0.7,headwidth=2.5, headlength=2.5, headaxislength=2.4)
        
        i+=1

    ax33.set_ylim([peakperiod.min()-5,peakperiod.max()+10])
    ...
    ax66.grid(True)
    ax66.plot(time.values,meanperiod.values, color='black', label='Mean Period')
    ...
    for x in time:
        ...
        change = [speed * math.cos(angle), speed * math.sin(angle)]
        if is_multiple_of_three(i):
            ax66.quiver(x, meanperiod[i], change[0], change[1], color='lightblue',scale=25,width=0.01, edgecolor='black', linewidth=0.7,headwidth=2.5, headlength=2.5, headaxislength=2.4)
        
        i+=1

    ax66.set_ylim([meanperiod.min()-5,meanperiod.max()+10])
    ...
    table.scale(1, 0.7)
    #arrname = ['Hs','Significant Wave Height','sea_Tp','Sea peak period']
    #ax6.table(cellText=df_details.values, colLabels=arrname, loc='center',
    #          colWidths=[0.15 for col in range(df.columns.size)], colColours=["white"] * 4)
    ax6.axis('off')
...
def create_table():
    ...
    df = pd.DataFrame({'DateTime':time,'Hs':hs, 'Tp':peakperiod,'PkDir':peakdir,'Mwd':dir, 'Tm':meanperiod,'Wsp': WSPD.values,'Gst':gust_df, 'WD':winDirectionDegrees, 'Vis':vis_df, 'SST':sst_df})
    decimals = 1  
...
def mslp_figure():
    ...
    ax2.text(0.76, -0.060,"Copyright 2023 SPC", transform=ax.transAxes,fontsize=8, verticalalignment='top')
    return fig
    ...
    with PdfPages(report_name) as pdf:
        ...
        for table in df_list:
            ...
            for _, row in table.iterrows():
                ...
                #colors_in_column[2] =get_color_from_jet(row["Tm"],0,20)
            ...
            ax.text(0.76, -0.090,"Copyright 2023 SPC", transform=ax.transAxes,fontsize=8, verticalalignment='top')
            pdf.savefig()
